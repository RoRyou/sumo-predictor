"""Siamese with close-rank-weighted loss — focus capacity on the hard 60% of test bouts."""
...
def main():
    ...
    p.add_argument("--close-rank-thresh", type=int, default=25)
    p.add_argument("--close-rank-mult", type=float, default=3.0)
    p.add_argument("--out", default="runs/siamese_focused_probs.npz")
    args = p.parse_args()
    ...
    # Build sample weights focusing on close-rank bouts
    close_mask_tr = train_df["rank_diff"].abs() < args.close_rank_thresh
    # New sample weight: base * (mult if close-rank else 1)
    sw_focused = train_df["sample_weight"].values * np.where(close_mask_tr, args.close_rank_mult, 1.0)
    print(f"  close-rank train ratio: {close_mask_tr.mean():.1%}")
    print(f"  weight mult: close={args.close_rank_mult}, far=1.0")

    sc_a = StandardScaler().fit(train_df[side_cols].fillna(0))
    sc_p = StandardScaler().fit(train_df[pair_cols].fillna(0))
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")

    def to_t(d, sw_override=None):
        ...
        sw_vals = sw_override if sw_override is not None else d["sample_weight"].values
        sw = torch.tensor(sw_vals.astype(np.float32), dtype=torch.float32).to(device)
        return x_a, x_b, id_a, id_b, x_p, y, sw

    train = to_t(train_df, sw_override=sw_focused)
    val = to_t(val_df); test = to_t(test_df)

    val_probs, test_probs = [], []
    for seed in range(args.seeds):
        torch.manual_seed(seed); np.random.seed(seed)
        model = Siamese(n_side=len(side_cols), n_pair=len(pair_cols), n_rikishi=n_rikishi).to(device)
