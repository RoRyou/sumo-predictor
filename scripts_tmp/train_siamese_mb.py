"""Siamese deep with multi-basho validation.
train: bashoId < 202307
val: bashoId in {202307, 202309, 202311}
test: bashoId >= 202401
"""
...
def main():
    ...
    p.add_argument("--out", default="runs/siamese_mb_probs.npz")
    args = p.parse_args()

    df = pd.read_parquet(args.features)
    df["bashoId"] = df["bashoId"].astype(str)
    train_df = df[df["bashoId"] < args.train_end].copy()
    val_df = df[df["bashoId"].isin(args.val_bashos)].copy()
    ...
    print(f"  train={len(train_df)} val={len(val_df)} test={len(test_df)} rikishi={n_rikishi}")
    ...
    print(f"\nMulti-basho Bag-{args.seeds}: val_acc={accuracy_score(y_va_np, v>0.5):.4f} val_auc={roc_auc_score(y_va_np, v):.4f}")
    ...
    np.savez(args.out, val=v, test=t, val_iso=v_iso, test_iso=t_iso,
             ...
             val_bashos=val_df["bashoId"].values.astype('U10'))
