"""Siamese with mixup augmentation. λ ~ Beta(0.2, 0.2) blends two bouts.

For each batch, pair each sample with a random other sample, mix inputs & targets.
This smooths the decision boundary and acts as strong regularizer.

Note: rikishi IDs can't be mixed (they're categorical). For simplicity we keep
sample A's IDs but mix the continuous features.
"""
...
def main():
    ...
    p.add_argument("--alpha", type=float, default=0.2, help="mixup alpha")
    p.add_argument("--out", default="runs/siamese_mixup_probs.npz")
    args = p.parse_args()
    ...
    print(f"  side={len(side_cols)} pair={len(pair_cols)} rikishi={n_rikishi} mixup_α={args.alpha}")
    ...
    for seed in range(args.seeds):
        ...
        model = SiameseMixup(n_side=len(side_cols), n_pair=len(pair_cols), n_rikishi=n_rikishi).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, n_tr, args.batch):
                ...
                # Mixup with random permutation (within batch)
                lam = float(np.random.beta(args.alpha, args.alpha))
                perm_inner = torch.randperm(len(idx), device=device)
                idx2 = idx[perm_inner]
                xa_m = lam * x_a_tr[idx] + (1 - lam) * x_a_tr[idx2]
                xb_m = lam * x_b_tr[idx] + (1 - lam) * x_b_tr[idx2]
                xp_m = lam * x_p_tr[idx] + (1 - lam) * x_p_tr[idx2]
                y_m = lam * y_tr[idx] + (1 - lam) * y_tr[idx2]
                sw_m = lam * sw_tr[idx] + (1 - lam) * sw_tr[idx2]
                opt.zero_grad()
                # Use original IDs for sample (mixing IDs makes no sense)
                logits = model(xa_m, xb_m, id_a_tr[idx], id_b_tr[idx], xp_m)
                loss = (loss_fn(logits, y_m) * sw_m).mean()
                loss.backward()
                opt.step()
            model.eval()
            with torch.no_grad():
