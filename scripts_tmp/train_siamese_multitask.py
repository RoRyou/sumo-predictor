"""Multi-task Siamese: winner (binary) + kimarite (74-way).

Shared encoder, two heads.
Joint loss: BCE(winner) + λ * CrossEntropy(kimarite)
λ chosen so kimarite gradient is comparable in magnitude.

Hypothesis: kimarite prediction is a richer signal (74 classes vs 2),
provides regularization and forces the encoder to learn more nuanced
representations of rikishi styles + bout dynamics.
"""
...
class MultiTaskSiamese(nn.Module):
    def __init__(self, n_side, n_pair, n_rikishi, n_kimarite, emb_id=16, hidden=96, dropout=0.20):
        super().__init__()
        ...
        # Kimarite head (74-way)
        self.kimarite_head = nn.Linear(64, n_kimarite)
    ...
    def forward(self, x_a, x_b, id_a, id_b, x_pair):
        ...
        h = self.trunk(z)
        return self.winner_head(h).squeeze(-1), self.kimarite_head(h)
...
def main():
    ...
    p.add_argument("--lambda-kimarite", type=float, default=0.2,
                   help="weight of kimarite cross-entropy loss")
    p.add_argument("--out", default="runs/siamese_multitask_probs.npz")
    args = p.parse_args()
    ...
    # Kimarite encoding
    all_kimarites = sorted(df["kimarite"].unique())
    kimarite_map = {k: i for i, k in enumerate(all_kimarites)}
    n_kimarite = len(all_kimarites)
    print(f"  side={len(side_cols)} pair={len(pair_cols)} rikishi={n_rikishi} kimarite={n_kimarite}")
    ...
    def to_t(d):
        ...
        k = torch.tensor([kimarite_map[ki] for ki in d["kimarite"]], dtype=torch.long).to(device)
        sw = torch.tensor(d["sample_weight"].values, dtype=torch.float32).to(device)
        return x_a, x_b, id_a, id_b, x_p, y, k, sw
    ...
    for seed in range(args.seeds):
        ...
        model = MultiTaskSiamese(n_side=len(side_cols), n_pair=len(pair_cols),
                                  n_rikishi=n_rikishi, n_kimarite=n_kimarite).to(device)
        opt = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
        bce = nn.BCEWithLogitsLoss(reduction="none")
        ce = nn.CrossEntropyLoss(reduction="none")

        x_a_tr, x_b_tr, id_a_tr, id_b_tr, x_p_tr, y_tr, k_tr, sw_tr = train
        x_a_va, x_b_va, id_a_va, id_b_va, x_p_va, y_va, _, _ = val
        x_a_te, x_b_te, id_a_te, id_b_te, x_p_te, y_te, _, _ = test
        n_tr = len(y_tr)
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, n_tr, args.batch):
                ...
                opt.zero_grad()
                w_logit, k_logit = model(x_a_tr[idx], x_b_tr[idx], id_a_tr[idx], id_b_tr[idx], x_p_tr[idx])
                w_loss = (bce(w_logit, y_tr[idx]) * sw_tr[idx]).mean()
                k_loss = (ce(k_logit, k_tr[idx]) * sw_tr[idx]).mean()
                loss = w_loss + args.lambda_kimarite * k_loss
                loss.backward()
                opt.step()
            model.eval()
            with torch.no_grad():
                w_va, _ = model(x_a_va, x_b_va, id_a_va, id_b_va, x_p_va)
                p_va = torch.sigmoid(w_va).cpu().numpy()
        ...
        with torch.no_grad():
            w_va, _ = model(x_a_va, x_b_va, id_a_va, id_b_va, x_p_va)
            p_va = torch.sigmoid(w_va).cpu().numpy()
            w_te, _ = model(x_a_te, x_b_te, id_a_te, id_b_te, x_p_te)
            p_te = torch.sigmoid(w_te).cpu().numpy()
    ...
    y_va_np = val[5].cpu().numpy(); y_te_np = test[5].cpu().numpy()
