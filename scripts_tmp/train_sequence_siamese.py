"""Sequence Siamese: Transformer encodes each rikishi's last 20-bout history.

For each rikishi:
- 20-step history sequence (4 features per step: won, opp_rank, days_since, pos)
- Linear project each step to d_model
- Add positional encoding
- 2-layer Transformer encoder
- Attention pool to 32-dim rikishi state representation

Then siamese head on [state_A, state_B, |Δ|, ⊙, pair_features].

This captures TEMPORAL PATTERNS that aggregated features (mean/std) lose:
- "Rikishi A is on a 5-loss streak then 3-win recovery"
- "Has been losing to high-ranked opponents lately"
- "Recent kimarite shifts (style adaptation)"
"""
...
K = 20  # history length
F_SEQ = 4  # features per step


class SequenceEncoder(nn.Module):
    """Encode a (K, F_SEQ) history sequence into a single d-dim vector."""
    def __init__(self, d_model=32, n_heads=4, n_layers=2, dropout=0.20):
        super().__init__()
        self.proj = nn.Linear(F_SEQ, d_model)
        self.pos_emb = nn.Parameter(torch.randn(K, d_model) * 0.02)
    ...
    def forward(self, x_seq):
        # x_seq: (B, K, F_SEQ)
        h = self.proj(x_seq) + self.pos_emb.unsqueeze(0)
        h = self.encoder(h)
        # Attention pool
        B = x_seq.size(0)
...
class SequenceSiamese(nn.Module):
    def __init__(self, n_side, n_pair, n_rikishi, emb_id=16, d_seq=32, hidden=96, dropout=0.20):
        super().__init__()
        # Standard tower for static features
        ...
        # Shared sequence encoder for both sides
        self.seq_enc = SequenceEncoder(d_model=d_seq, dropout=dropout)
        # Rikishi ID
        ...
        # Head
        side_dim = 48 + emb_id + d_seq  # static + id + sequence
    ...
    def _encode(self, x_feat, x_seq, x_id):
        return torch.cat([
            self.feat_tower(x_feat),
            self.id_emb(x_id),
            self.seq_enc(x_seq),
        ], dim=-1)

    def forward(self, x_a, seq_a, x_b, seq_b, id_a, id_b, x_pair):
        e_a = self._encode(x_a, seq_a, id_a)
        e_b = self._encode(x_b, seq_b, id_b)
...
def main():
    ...
    p.add_argument("--history", default="data/processed/rikishi_history_seq.parquet")
    p.add_argument("--seeds", type=int, default=10)
    p.add_argument("--epochs", type=int, default=60)
    p.add_argument("--batch", type=int, default=256)
    p.add_argument("--out", default="runs/sequence_siamese_probs.npz")
    args = p.parse_args()

    df = pd.read_parquet(args.features)
    df["bashoId"] = df["bashoId"].astype(str)
    seq = pd.read_parquet(args.history)
    seq["bashoId"] = seq["bashoId"].astype(str)
    df = df.merge(seq, on=["bashoId", "day", "matchNo"], how="left")
    # Fill missing history with zeros
    east_cols = [f'east_h{j}_f{f}' for j in range(K) for f in range(F_SEQ)]
    west_cols = [f'west_h{j}_f{f}' for j in range(K) for f in range(F_SEQ)]
    for c in east_cols + west_cols:
        df[c] = df[c].fillna(0)
    print(f"  merged df: {len(df)} rows, with history")
    ...
    # MPS has issues with transformer; use CPU
    ...
    def to_t(d):
        ...
        # Sequences: (N, K, F_SEQ)
        seq_a = torch.tensor(d[east_cols].values.reshape(-1, K, F_SEQ), dtype=torch.float32).to(device)
        seq_b = torch.tensor(d[west_cols].values.reshape(-1, K, F_SEQ), dtype=torch.float32).to(device)
        ...
        return x_a, seq_a, x_b, seq_b, id_a, id_b, x_p, y, sw
    ...
    for seed in range(args.seeds):
        ...
        model = SequenceSiamese(n_side=len(side_cols), n_pair=len(pair_cols),
                                 n_rikishi=n_rikishi).to(device)
        opt = optim.AdamW(model.parameters(), lr=8e-4, weight_decay=1e-4)
        sched = optim.lr_scheduler.CosineAnnealingLR(opt, T_max=args.epochs)
        loss_fn = nn.BCEWithLogitsLoss(reduction="none")

        x_a_tr, seq_a_tr, x_b_tr, seq_b_tr, id_a_tr, id_b_tr, x_p_tr, y_tr, sw_tr = train
        x_a_va, seq_a_va, x_b_va, seq_b_va, id_a_va, id_b_va, x_p_va, y_va, _ = val
        x_a_te, seq_a_te, x_b_te, seq_b_te, id_a_te, id_b_te, x_p_te, y_te, _ = test
        n_tr = len(y_tr)
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, n_tr, args.batch):
                ...
                opt.zero_grad()
                logits = model(x_a_tr[idx], seq_a_tr[idx], x_b_tr[idx], seq_b_tr[idx],
            ...
            model.eval()
            with torch.no_grad():
                p_va = torch.sigmoid(model(x_a_va, seq_a_va, x_b_va, seq_b_va,
                                            id_a_va, id_b_va, x_p_va)).cpu().numpy()
        ...
        with torch.no_grad():
            p_va = torch.sigmoid(model(x_a_va, seq_a_va, x_b_va, seq_b_va,
                                        id_a_va, id_b_va, x_p_va)).cpu().numpy()
            p_te = torch.sigmoid(model(x_a_te, seq_a_te, x_b_te, seq_b_te,
                                        id_a_te, id_b_te, x_p_te)).cpu().numpy()
