"""Wide & Deep model: linear memorization + deep generalization, jointly trained.

Architecture:
- Wide: linear sum over all features (memorize feature-target associations)
- Deep: Siamese network (per-side tower + ID embedding)
- Final logit = wide_logit + deep_logit (joint loss back-propagates to both)
"""
...
class WideDeep(nn.Module):
    def __init__(self, n_side, n_pair, n_rikishi, emb_id=16, hidden=96):
        super().__init__()
        # Wide: linear on all features (concat A + B + pair)
        self.wide = nn.Linear(2 * n_side + n_pair, 1, bias=False)
        # Deep: siamese
    ...
    def forward(self, x_a, x_b, id_a, id_b, x_pair):
        # Wide
        wide_logit = self.wide(torch.cat([x_a, x_b, x_pair], dim=-1)).squeeze(-1)
        # Deep
        ...
        deep_logit = self.deep_head(z).squeeze(-1)
...
def main():
    ...
    p.add_argument("--out", default="runs/wide_deep_probs.npz")
    args = p.parse_args()
    ...
    for seed in range(args.seeds):
        ...
        model = WideDeep(n_side=len(side_cols), n_pair=len(pair_cols), n_rikishi=n_rikishi).to(device)
        # Separate optimizer for wide (heavy L2) and deep
        opt = optim.AdamW([
            {'params': model.wide.parameters(), 'weight_decay': 1.0},
            {'params': [p for n, p in model.named_parameters() if not n.startswith('wide.')], 'weight_decay': 1e-4},
        ], lr=1e-3)
