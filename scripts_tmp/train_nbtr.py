"""Neural Bradley-Terry Rating (NBTR) — single scalar skill per rikishi.

Architecture:
- Per-rikishi: skill_score = MLP(features_per_side, rikishi_id_emb) → 1 scalar
- Pair adjustment: MLP(pair_features) → 1 scalar
- Logit = skill_east - skill_west + pair_adjustment + east_advantage_bias
- P(east wins) = sigmoid(logit)

Key difference from Siamese:
- Siamese: 64-dim per-rikishi vector → concat/diff/prod head (expressive)
- NBTR: 1-dim per-rikishi rating + pair MLP (constrained, BT-aligned)

The constraint may regularize better at small data scale.
"""
...
class NBTR(nn.Module):
    """Neural Bradley-Terry Rating with asymmetric pair adjustment."""
    def __init__(self, n_side, n_pair, n_rikishi, emb_id=16, hidden=64, dropout=0.20):
        super().__init__()
        # Shared per-rikishi skill encoder → 1 scalar
        self.skill_tower = nn.Sequential(
            nn.Linear(n_side, hidden), nn.LayerNorm(hidden), nn.GELU(), nn.Dropout(dropout),
            nn.Linear(hidden, 32), nn.LayerNorm(32), nn.GELU(), nn.Dropout(dropout * 0.75),
            nn.Linear(32, 1),
        )
        # Rikishi ID also contributes to skill (additive)
        self.id_skill = nn.Embedding(n_rikishi + 1, 1)
        nn.init.zeros_(self.id_skill.weight)
        # Pair adjustment: how much does the pair context tilt the outcome?
        self.pair_adj = nn.Sequential(
            nn.Linear(n_pair, hidden), nn.LayerNorm(hidden), nn.GELU(), nn.Dropout(dropout),
            nn.Linear(hidden, 32), nn.LayerNorm(32), nn.GELU(),
            nn.Linear(32, 1),
        )
        # East-side advantage bias (learnable scalar)
        self.east_bias = nn.Parameter(torch.zeros(1))

    def _skill(self, x_feat, x_id):
        return self.skill_tower(x_feat).squeeze(-1) + self.id_skill(x_id).squeeze(-1)

    def forward(self, x_a, x_b, id_a, id_b, x_pair):
        s_e = self._skill(x_a, id_a)
        s_w = self._skill(x_b, id_b)
        adj = self.pair_adj(x_pair).squeeze(-1)
        return s_e - s_w + adj + self.east_bias
...
def main():
    ...
    p.add_argument("--out", default="runs/nbtr_probs.npz")
    args = p.parse_args()
    ...
    for seed in range(args.seeds):
        ...
        model = NBTR(n_side=len(side_cols), n_pair=len(pair_cols), n_rikishi=n_rikishi).to(device)
