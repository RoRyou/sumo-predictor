"""Siamese with FEATURE DROPOUT augmentation during training.

Randomly mask 10-30% of input features per batch (replace with zero post-scaling).
This makes the model more robust to feature noise and may regularize better
than weight decay alone.
"""
...
class SiameseAug(nn.Module):
    def __init__(self, n_side, n_pair, n_rikishi, emb_id=16, hidden=96):
        super().__init__()
...
def main():
    ...
    p.add_argument("--feat-drop", type=float, default=0.15,
                   help="probability of zeroing each feature during training")
    p.add_argument("--out", default="runs/siamese_aug_probs.npz")
    args = p.parse_args()
    ...
    print(f"  side={len(side_cols)} pair={len(pair_cols)} rikishi={n_rikishi} feat_drop={args.feat_drop}")
    ...
    for seed in range(args.seeds):
        ...
        model = SiameseAug(n_side=len(side_cols), n_pair=len(pair_cols), n_rikishi=n_rikishi).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, n_tr, args.batch):
                ...
                # Feature dropout: per-batch, per-feature mask
                mask_a = (torch.rand(x_a_tr[idx].shape, device=device) > args.feat_drop).float()
                mask_b = (torch.rand(x_b_tr[idx].shape, device=device) > args.feat_drop).float()
                mask_p = (torch.rand(x_p_tr[idx].shape, device=device) > args.feat_drop).float()
                opt.zero_grad()
                logits = model(
                    x_a_tr[idx] * mask_a, x_b_tr[idx] * mask_b,
                    id_a_tr[idx], id_b_tr[idx], x_p_tr[idx] * mask_p,
                )
